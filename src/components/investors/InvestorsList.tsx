  ...
  const handleAddToSubscriptions = async (projectId: string) => {
    ...
    try {
      ...
      try {
        ...
        await logActivity({
          action_type: "add_investors_to_subscriptions",
          details: {
            project_id: projectId,
            investor_count: selectedInvestors.length,
            investor_ids: selectedInvestors,
          },
          // We're not including entity_type and entity_id here to avoid potential errors
          // The activityLogger will handle this dynamically based on schema availability
        });
      } catch (logError) {
